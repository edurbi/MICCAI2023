...
class Model(nn.Module):
    ...
    def forward(self, x, batch,rad_1,rad_2,training=True):

        featuresx = self.flair_encoder(x)
        featurest,_=self.flair_transform(featuresx)
        featurest=featurest.view(-1,256,256)
        feat = featurest[None, ...].permute(1, 0, 2, 3)
        if training:
            feat=self.random_elimination(feat,0.05)

        rad_1 = self.reducer(rad_1)
        rad_2 = self.reducer2(rad_2)
        rad_1f = torch.cat((rad_1,feat),1)
        rad_2f = torch.cat((rad_2, feat), 1)
        ...
        if training :
            conv_X1 = self.random_elimination(conv_X1,0.05)
        if training :
            conv_X2 = self.random_elimination(conv_X2, 0.05)
        if training:
            conv_X3 = self.random_elimination(conv_X3,0.05)


        x_final = torch.cat((conv_X2, conv_X1, feat, rad_1, rad_2,conv_X3), dim=1)
        #x_final = torch.cat((conv_X2, conv_X1, feat, rad_1, rad_2), dim=1)
        #x_final = torch.cat((conv_X2, conv_X1, feat), dim=1)
        preds= self.final1(x_final)
        return preds